...
class SS_Conv_SSM_3D(nn.Module): #no multiplicative path, added MLP. more like transformer block used in TABSurfer now
  def __init__(
    ...
    self.self_attention = SS3D_v5(d_model=hidden_dim//2, 
...
class MedSegMamba(nn.Module):
    def __init__(
        ...
        self.Up_conv4 = ResConvBlock3D(ch_in=channel_sizes[3]+channel_sizes[3], ch_out=channel_sizes[3], id=id, preact=preact)
