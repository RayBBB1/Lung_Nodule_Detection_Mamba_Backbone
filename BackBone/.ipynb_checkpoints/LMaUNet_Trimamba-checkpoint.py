...
class ResidualBiMambaEncoder(nn.Module):
    def __init__(self,
        ...
        w_mamba_layers = []
        mamba_layers = []
        num_slices_list = [48, 24, 12]
        window_size_list = [8, 8, 4]
        pool_list = [2, 2, 2]
        for s in range(n_stages):
            ...
            mamba_layers.append(TriPixelMambaLayer(
                    dim=input_channels,
                    p=num_slices_list[s] // window_size_list[s],
                    num_slices=window_size_list[s]
                ))
            w_mamba_layers.append(TriWindowMambaLayer(
                    dim=input_channels,
                    p=pool_list[s],
                    num_slices=num_slices_list[s] // pool_list[s]
                ))
