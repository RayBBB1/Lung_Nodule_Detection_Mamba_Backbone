...
class SS_Conv_SSM_3D(nn.Module): # 維持原始類名
    def __init__(
        ...
        **kwargs,
        ...
        # *** 添加絕對位置嵌入 (APE) ***
        self.window_volume = self.window_size[0] * self.window_size[1] * self.window_size[2]
        # 創建可學習的位置嵌入參數，形狀為 (1, 窗口體積, 通道數一半)
        self.pos_embed_attn = nn.Parameter(torch.zeros(1, self.window_volume, dim_half))
        # 使用截斷正態分佈初始化
        trunc_normal_(self.pos_embed_attn, std=.02)
        # ***************************
    ...
    def forward(self, x: torch.Tensor):
        ...
        #   d.1 添加位置嵌入
        #       pos_embed_attn: (1, window_volume, C/2)
        #       使用廣播機制將其加到每個窗口上
        attn_input = right_windows + self.pos_embed_attn

        #   d.2 應用注意力
        attn_windows = self.attn(attn_input) # B*nW, win_vol, C/2
